...
Before you start, be sure to load these packages, you could also use the command `require()`:
...
##Reference point
Next week, I'll talk to you about participation levels. To get a sense of what you can do with data, take a look at this graph at FRED: 
...
I would recommend that you save it in your Behavioral Economics folder under a folder called Lab2 and then create a backup of the file in that folder too. 
Open the file if you want to see what a csv looks like in Excel or Google Sheets. 
Do not save any changes you make to it. 

Now, we are going to open the file in R using RStudio. 
Use the command below, but be sure to change the filepath to the one where you have put the `labor_force_levels.csv` file. 
Note, that in the filepath below I put a `.` at the start to tell R to look 'back' one level (to one level up in the folder structure). 
