...
variety of fantastic data available to the public. In Labs 1 and 2, we saw a table
...
You can see the FRED graph of labor force participation rate at the following link:
...
value (csv) data, we need to use one of two commands (which we saw in the previous lab): 
...
filepath where the file is. the `readr` package using `read_csv()` is very useful for big csv files or for doing it more quickly (along with greater customization). 
...
```{r}
Participation <- read_csv("../more/labor_force_participation.csv", 
...
OR... you can read the data in with the `readxl()` command from the `tidyverse`. Note: you may need to install `readxl` (install it using `install.packages("readxl")`)
...
**Remember**: If you have the csv/Excel file *in the same folder* as your R Markdown file, then you don't need to specify the folders `../more` and so on, you could just have: `ParticipationXL <- read_excel("labor_force_participation.xls")`. 
...
use. 
We shall do this by putting the data on a line _before_ we call on the
ggplot command and we shall follow the name of the data with the following
symbol: `%>%`. 
Remember from last week that the `%>%` symbol is called a __pipe__ in programming, statistics or econometrics. 
You should read the pipe as saying "Then" or "And then". 
...
We want a line plot. 
To make a plot we have to 
specify a kind of `geom` for `ggplot`. 
A `geom` is a type of object that talks to R and says "Make Me A Line Plot" 
or "Make Me A Scatter Plot" or "Make Me a Density Plot". 
We shall use `geom_line` as a line graph is the one we want. 
Check the notes on using `ggplot` if you want to be sure of this (see the Moodle uploads for these documents or the `ggplot` cheatsheet under the help menu). 

Now, `ggplot` also needs you to end each line with a `+` sign. 
Think backto the 6-year-old from Lab 2 who was telling you about the `%>%` symbol:  she might say 'and' a lot. 
We have to treat ggplot like a 6-year-old who says 'and' a lot. 
So it doesn't just want to be told what the x-axis and y-axis are, it also needs to be told "and I want a line plot".
...
-   You only need to change two parts of the code above. 
...
__Tidy data__ typically has unique __variables__ in each column
and unique __cases__ in each row. 
So let's take a moment to think about the data that we have.  
...
`gather` it.
`gather` is a very useful command in the `dplyr` package (part of the `tidyverse`) that allows us to change the cases and variables we're interested in analyzing. 
...
We can do this in a similar, but slightly different, way as follows: 
```{r}
PartGatherType2 <- #name the new object
  Participation %>% #pipe in the data
  gather(sex, rate, -date) #use the variables that AREN'T the date
```
...
graphs did not. This happened because I specified an option called `ylim()` (the limites of the y-axis) with 0 as the low end and NA (not applicable) as the top end. 
...
```{r dygraph, echo = FALSE, warning=FALSE, message = FALSE}
library(xts)
library(dygraphs)
...
starting with a certain letter or word. If we have time, we shall look at data from GapMinder in order to test out our ideas. You'll also pracise on DataCamp. 
